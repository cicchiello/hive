...
public class LocalStorageHandler extends SQLiteOpenHelper {
	...
	private static final String DATABASE_NAME = "IoTeddyIM.db";
	...
	public void delete(String sender, String receiver) {
		Cursor c = get(sender, receiver);
	    Log.i(TAG, "Deleting "+c.getCount()+" entries");

	    SQLiteDatabase db = getWritableDatabase();
	    int cnt = db.delete(TABLE_NAME_MESSAGES, MESSAGE_SENDER + " LIKE '" + sender + "' AND " + MESSAGE_RECEIVER + " LIKE '" + receiver + "'", null);
	    Log.i(TAG, "db.delete reports "+cnt);
	    
		c = get(sender, receiver);
	    Log.i(TAG, "There are "+c.getCount()+" matching entries left");
	}
	
	public void deleteAll() {
	    SQLiteDatabase db = getWritableDatabase();
	    int cnt = db.delete(TABLE_NAME_MESSAGES, "1", null);
	    Log.i(TAG, "db.delete reports "+cnt);
	}
