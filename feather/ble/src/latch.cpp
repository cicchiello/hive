...
Latch::Latch(const char *name, const class RateProvider &rateProvider, unsigned long now, int servoPin,
	     const TempSensor &tempSensor, const ServoConfig &servoConfig)
  : Sensor(name, rateProvider, now), mServoPin(servoPin), mTempSensor(tempSensor),
